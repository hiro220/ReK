...
class Beam(pygame.sprite.Sprite):
    def __init__(self, x, y, dx, dy, machines, principal, img):
        ...
        self.p_rect = principal                               #beamをする本人の位置情報
...
class Beam_principal(Beam):
    ...
    def update(self):
        if R_time.get_ticks() - self.gun_start >= 1000 and self.rect.width <= 600:                   #クロック数が1000以上とbeam本体の横の長さが600以下
        ...
        if R_time.get_ticks() - self.gun_start >= 1000 and self.rect.height > 0 and self.flag == 1:  
        ...
        x, y = self.p_rect.midleft                                                                        #打ったmachineの座標を所得する
...
class Beam_sub(Beam):  #サブクラス
    def __init__(self, x, y, dx, dy, machines, principal, img):
        ...
        #ここでサブ画像の大きさを大きくする
    def update(self):
        if  self.flag == 0 and self.count < 6:    #格納したデータを置き換えながら表示する
        ...
        if R_time.get_ticks() - self.gun_start >= 3900 and self.flag == 1 and self.rect.height > 0:
